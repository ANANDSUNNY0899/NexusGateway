...
// func (o *OpenAIAdapter) PrepareRequest(p, m, k, v string) (*http.Request, error) {
...
// PrepareRequest now accepts the full message slice []Message instead of a single string
func (o *OpenAIAdapter) PrepareRequest(messages []Message, model, apiKey, version string) (*http.Request, error) {
	url := "https://api.openai.com/v1/chat/completions"
	
	// Intelligent Routing to Groq for Llama/Mixtral models
	if strings.Contains(strings.ToLower(model), "llama") || strings.Contains(strings.ToLower(model), "mixtral") {
		url = "https://api.groq.com/openai/v1/chat/completions"
	}

	// Build payload with the entire conversation history
	payload := map[string]any{
		"model":    model,
		"messages": messages, // <--- This is where the magic happens
		"stream":   true,
	}
...
func (o *OpenAIAdapter) ParseStreamChunk(line string) (string, bool) {
	...
	cleanLine := strings.TrimPrefix(line, "data: ")
	if strings.TrimSpace(cleanLine) == "[DONE]" {
		return "", true
	}
